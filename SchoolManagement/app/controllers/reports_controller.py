...
class ReportsController:
    ...
    def get_reports(self):
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
                SELECT reports.id, reports.type_report_id, users.name AS reporter_name, 
                    reported_users.name AS reported_user_name, reports.description
                FROM reports
                JOIN users AS users ON reports.reporter_id = users.id
                JOIN users AS reported_users ON reports.reported_user_id = reported_users.id
            """)
            ...
            for data in result:
                content = {
                    ...
                    'reporter_name': data[2],
                    'reported_user_name': data[3],
                    'description': data[4]
                }
                payload.append(content)
            if payload:
    ...
    def edit_report(self, report_id: int, reports: dict):
        try:
            ...
            # Validar la entrada antes de ejecutar la consulta
            if 'type_report_id' not in reports or 'reporter_id' not in reports or 'reported_user_id' not in reports or 'description' not in reports:
